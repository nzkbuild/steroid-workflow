#!/usr/bin/env node
/**
 * standalone-compat.cjs — Compatibility runtime for standalone project installs.
 *
 * This file remains the standalone compatibility payload for initialized
 * projects. The public entrypoint in bin/steroid-run.cjs is now a thin wrapper
 * that prefers the modular runtime and falls back to this file only when
 * needed.
 *
...
} = require('../utils/fork-sources.cjs');
const {
    detectUiTask,
    detectAuditMode,
    detectStack: detectDesignStack,
    routeDesignSystems: routeCanonicalDesignSystems,
} = require('../utils/design-routing.cjs');
const {
    normalizeAllowedStatus: normalizeCanonicalAllowedStatus,
    normalizeDesignRoutingReceipt: normalizeCanonicalDesignRoutingReceipt,
    normalizeUiReviewReceipt: normalizeCanonicalUiReviewReceipt,
} = require('../utils/governed-receipts.cjs');
const {
    loadDesignRoutingReceipt: loadCanonicalDesignRoutingReceipt,
    loadUiReviewReceipt: loadCanonicalUiReviewReceipt,
} = require('../utils/frontend-receipt-loaders.cjs');
const {
    buildAccesslintResultFromReceipt: buildCanonicalAccesslintResultFromReceipt,
    normalizePreviewUrlCandidate: normalizeCanonicalPreviewUrlCandidate,
    refreshUiReviewArtifacts: refreshCanonicalUiReviewArtifacts,
    summarizeBrowserAuditResult: summarizeCanonicalBrowserAuditResult,
    detectUiFeatureForGate: detectCanonicalUiFeatureForGate,
} = require('../utils/frontend-review.cjs');
const {
    buildUiArchivePolicy: buildCanonicalUiArchivePolicy,
    summarizeUiReviewStatus: summarizeCanonicalUiReviewStatus,
} = require('../utils/ui-archive-policy.cjs');
const {
    loadCompletionReceipt: loadCanonicalCompletionReceipt,
    loadExecutionReceipt: loadCanonicalExecutionReceipt,
    loadRequestReceipt: loadCanonicalRequestReceipt,
    loadReviewReceipt: loadCanonicalReviewReceipt,
    loadVerifyReceipt: loadCanonicalVerifyReceipt,
    parseVerifyMarkdownStatus: parseCanonicalVerifyMarkdownStatus,
    saveCompletionReceipt: saveCanonicalCompletionReceipt,
    saveExecutionReceipt: saveCanonicalExecutionReceipt,
    saveRequestReceipt: saveCanonicalRequestReceipt,
    saveReviewReceipt: saveCanonicalReviewReceipt,
    saveVerifyReceipt: saveCanonicalVerifyReceipt,
} = require('../utils/receipt-loaders.cjs');
const {
    analyzePrompt: analyzeCanonicalPrompt,
    buildPromptHealth: buildCanonicalPromptHealth,
    inspectSession,
    normalizeWhitespace: normalizeCanonicalPromptWhitespace,
    ROUTE_PHASE_HINTS: CANONICAL_ROUTE_PHASE_HINTS,
    summarizeRouteProgress: summarizeCanonicalRouteProgress,
} = require('../utils/prompt-intelligence.cjs');
const {
    buildFeatureArtifactState: buildCanonicalFeatureArtifactState,
    getPipelineStatusEntries: getCanonicalPipelineStatusEntries,
} = require('../utils/pipeline-status.cjs');
const {
    bootstrapFeatureDesignArtifacts: bootstrapCanonicalFeatureDesignArtifacts,
    generateDesignSystemArtifact: generateCanonicalDesignSystemArtifact,
    resolveFeaturePromptForDesign: resolveCanonicalFeaturePromptForDesign,
} = require('../utils/design-workflow.cjs');
const {
    collectHtmlAuditTargets,
    resolveBrowserAuditTarget: resolveCanonicalBrowserAuditTarget,
} = require('../utils/browser-audit-target.cjs');
const { generateHandoffReportContent } = require('../utils/handoff-report.cjs');
const {
    createArchiveStamp: createCanonicalArchiveStamp,
    findBlockedShellSyntax: findCanonicalBlockedShellSyntax,
    getArchiveDestinationPath: getCanonicalArchiveDestinationPath,
    parseReviewMarkdown: parseCanonicalReviewMarkdown,
} = require('../utils/trust-helpers.cjs');
...
const runtimeAssetsRootDir =
    fs.existsSync(path.join(installedRuntimeAssetsDir, 'src', 'services')) ||
...
function readImportedManifest() {
    return readForkManifest(runtimeAssetsRootDir);
}
...
function getImportedSource(id) {
    return getForkSource(id, runtimeAssetsRootDir);
}
...
function resolveImportedSourcePath(id) {
    return resolveForkSourcePath(id, runtimeAssetsRootDir);
}
...
function routeDesignSystems(prompt, options = {}) {
    return routeCanonicalDesignSystems({
        prompt,
        stack: options.stack,
        auditOnly: options.auditOnly,
        rootDir: runtimeAssetsRootDir,
    });
}
...
function normalizeDesignRoutingReceipt(receipt) {
    return normalizeCanonicalDesignRoutingReceipt(receipt, { rootDir: runtimeAssetsRootDir });
}
...
function loadDesignRoutingReceipt(featureDir) {
    return loadCanonicalDesignRoutingReceipt(featureDir, { rootDir: runtimeAssetsRootDir });
}
...
function resolveFeaturePromptForDesign(featureDir) {
    return resolveCanonicalFeaturePromptForDesign(featureDir);
}

function generateDesignSystemMarkdown(query, options = {}) {
    return generateCanonicalDesignSystemArtifact(query, options);
}
...
function normalizePreviewUrlCandidate(value) {
    return normalizeCanonicalPreviewUrlCandidate(value);
}

function resolveBrowserAuditTarget(featureDir, htmlTargets = [], options = {}) {
    return resolveCanonicalBrowserAuditTarget(targetDir, featureDir, htmlTargets, options);
}

function summarizeBrowserAuditResult(audit) {
    return summarizeCanonicalBrowserAuditResult(audit);
}
...
function summarizeUiReviewStatus(findings) {
    return summarizeCanonicalUiReviewStatus(findings);
}

function buildUiArchivePolicy(uiReviewReceipt, options = {}) {
    return buildCanonicalUiArchivePolicy(uiReviewReceipt, options);
}

function normalizeUiReviewReceipt(receipt, feature) {
    return normalizeCanonicalUiReviewReceipt(receipt, feature);
}
...
function loadUiReviewReceipt(feature, featureDir) {
    return loadCanonicalUiReviewReceipt(feature, featureDir);
}

function buildAccesslintResultFromReceipt(receipt) {
    return buildCanonicalAccesslintResultFromReceipt(receipt);
}
...
function bootstrapFeatureDesignArtifacts(feature, featureDir, options = {}) {
    return bootstrapCanonicalFeatureDesignArtifacts(feature, featureDir, options);
}
...
function findBlockedShellSyntax(input) {
    return findCanonicalBlockedShellSyntax(input);
}
...
function parseReviewMarkdown(content) {
    return parseCanonicalReviewMarkdown(content);
}
...
function normalizeAllowedStatus(value, allowed, fallback) {
    return normalizeCanonicalAllowedStatus(value, allowed, fallback);
}
...
function generateHandoffReport(feature, featureDir, state, options = {}) {
    return generateHandoffReportContent({
        feature,
        version: SW_VERSION,
        ...
        verifyReceipt: normalizeUiReviewReceipt ? readLatestFeatureJsonArtifact(featureDir, 'verify.json') : readLatestFeatureJsonArtifact(featureDir, 'verify.json'),
        planContent: readLatestFeatureArtifact(featureDir, 'plan.md'),
        reviewContent: readLatestFeatureArtifact(featureDir, 'review.md'),
        uiReviewReceipt: normalizeUiReviewReceipt(readLatestFeatureJsonArtifact(featureDir, 'ui-review.json'), feature),
        state,
    });
}
...
function loadReviewReceipt(feature, featureDir) {
    return loadCanonicalReviewReceipt(feature, featureDir);
}
...
function saveReviewReceipt(featureDir, receipt) {
    return saveCanonicalReviewReceipt(featureDir, receipt);
}
...
function createArchiveStamp(date = new Date()) {
    return createCanonicalArchiveStamp(date);
}
...
function getArchiveDestinationPath(archiveDir, archiveStamp, fileName) {
    return getCanonicalArchiveDestinationPath(archiveDir, archiveStamp, fileName, fs.existsSync);
}
...
function parseVerifyMarkdownStatus(content) {
    return parseCanonicalVerifyMarkdownStatus(content);
}
...
function loadVerifyReceipt(feature, featureDir) {
    return loadCanonicalVerifyReceipt(feature, featureDir);
}
...
function saveVerifyReceipt(featureDir, receipt) {
    return saveCanonicalVerifyReceipt(featureDir, receipt);
}
...
function saveRequestReceipt(featureDir, receipt) {
    return saveCanonicalRequestReceipt(featureDir, receipt);
}
...
function loadRequestReceipt(feature, featureDir) {
    return loadCanonicalRequestReceipt(feature, featureDir);
}
...
function saveCompletionReceipt(featureDir, receipt) {
    return saveCanonicalCompletionReceipt(featureDir, receipt);
}
...
function loadCompletionReceipt(feature, featureDir) {
    return loadCanonicalCompletionReceipt(feature, featureDir);
}
...
function loadExecutionReceipt(feature, featureDir) {
    return loadCanonicalExecutionReceipt(feature, featureDir);
}
...
function saveExecutionReceipt(featureDir, receipt) {
    return saveCanonicalExecutionReceipt(featureDir, receipt);
}
...
function normalizePromptWhitespace(value) {
    return normalizeCanonicalPromptWhitespace(value);
}
...
function inspectPromptSessionState() {
    ...
    return inspectSession(featureStates, state);
}

function analyzePrompt(message, sessionState = inspectPromptSessionState()) {
    return analyzeCanonicalPrompt(message, sessionState);
}

function buildPromptHealth(analysis) {
    return buildCanonicalPromptHealth(analysis);
}

function summarizeRouteProgress(analysis, artifacts) {
    return summarizeCanonicalRouteProgress(analysis, artifacts);
}

function buildFeatureArtifactState(featureDir) {
    return buildCanonicalFeatureArtifactState(featureDir);
}

function getRouteDisplayPhases(route) {
    const phases = CANONICAL_ROUTE_PHASE_HINTS[route] || CANONICAL_ROUTE_PHASE_HINTS['standard-build'];
    return phases.map((phase) => (phase === 'normalize-prompt' ? 'prompt' : phase));
}

function getPipelineStatusEntries(featureDir, promptReceipt) {
    return getCanonicalPipelineStatusEntries(featureDir, promptReceipt);
}
...
Usage:
  ...
  Pipeline Enforcement:
    node steroid-run.cjs start <slug>                      Initialize a feature and bootstrap scan
    node steroid-run.cjs next <feature>                    Show route, blockers, and the next action
    node steroid-run.cjs finish <feature>                  Check review, verify, and archive readiness
    node steroid-run.cjs init-feature <slug>               Advanced: create feature folder structure only
...
if (args[0] === 'scan') {
    ...
    const featureDir = path.join(changesDir, feature);
    if (!fs.existsSync(featureDir)) {
        console.error(`[steroid-run] ❌ Feature "${feature}" not found. Run: npx steroid-run start ${feature}`);
        process.exit(1);
    }
...
if (args[0] === 'normalize-prompt') {
    ...
    if (args.includes('--write')) {
        ...
        if (!fs.existsSync(featureDir)) {
            console.error(
                `[steroid-run] ❌ Feature "${feature}" not found. Run: npx steroid-run start ${feature}`,
            );
            process.exit(1);
        }
