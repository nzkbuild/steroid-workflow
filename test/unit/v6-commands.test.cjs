...
/**
 * Unit tests for v6.0.0 commands: fs-*, smoke-test, git-init, verify-feature, scan --force, allowlist.
...
fs.writeFileSync(path.join(memoryDir, 'execution_state.json'), JSON.stringify({
    error_count: 0,
    last_error: null,
    status: 'active',
}));
...
test('fs-cp copies file', () => {
    ...
    if (fs.readFileSync(path.join(tmpBase, 'dest-file.txt'), 'utf-8') !== 'hello') throw new Error('Content mismatch');
});
